...
function get_work($qid, $format_string = 'ntriples', $force = false, $debug = false)
{
	global $config;	
	...
	if (1)
	{
		...
		{
			$url = 'https://wikicite-search.herokuapp.com/api.php?id=' . $qid . '&format=jsonld';
			$json = get($url, '*.*');
...
//----------------------------------------------------------------------------------------
function get_work_jsonld_framed($qid)
{
	$triples = get_work($qid, 'ntriples');
